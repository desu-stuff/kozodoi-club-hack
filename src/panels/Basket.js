...
const validateTime = (s) => s.match(/^\s*([01][0-9]|2[0-3])[.: -]([0-9]|[0-5][0-9])\s*$/i)


const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  // whatever
  const persisted = JSON.parse(localStorage['persist-basket-state'] || '{}')

  const [ faster, setFaster ] = useState('faster' in persisted ? persisted.faster : true);
  const [ time, setTime ] = useState('time' in persisted ? persisted.time : '');
  const [ selfService, setSelfService ] = useState('selfService' in persisted ? persisted.faster : false);
    ...
    <div className="Place">
      ...
      <div className="Place__products-wrapper">
        ...
        <Link
          ...
          onClick={() => {
            localStorage['persist-basket-state'] = JSON.stringify({
              faster,
              time,
              selfService
            })
